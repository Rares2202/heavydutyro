...
match ($method) {
    ...
    'PATCH'  => updateWorkoutNotes($userId),
...
// ── PATCH ──────────────────────────────────────────
function updateWorkoutNotes(int $userId): void {
    $id    = (int)($_GET['id'] ?? 0);
    $data  = getJsonBody();
    $notes = $data['notes'] ?? null;

    if (!$id || $notes === null) {
        jsonResponse(['success' => false, 'message' => 'ID sau notițe lipsă'], 400);
    }

    $db   = getDB();
    $stmt = $db->prepare('UPDATE workout_journal SET notes = ? WHERE id = ? AND user_id = ?');
    $stmt->execute([$notes, $id, $userId]);

    if ($stmt->rowCount() === 0) {
        $check = $db->prepare('SELECT COUNT(*) FROM workout_journal WHERE id = ? AND user_id = ?');
        $check->execute([$id, $userId]);
        if ($check->fetchColumn() === 0) {
            jsonResponse(['success' => false, 'message' => 'Antrenament negăsit'], 404);
        }
        jsonResponse(['success' => true, 'message' => 'Notițele sunt deja actualizate']);
    }

    jsonResponse(['success' => true, 'message' => 'Notițele au fost actualizate']);
}
